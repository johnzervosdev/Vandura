...
**Last Updated:** 2026-05-11 (Story **6.4** hide **`completed`** on project task table ✅ — **`npm test` 176** all pass; Story **6.3** ✅; Story **6.2** ✅; Story **6.5** ✅; Story **6.7** / **BUG-REPORT-001** ✅; **`npm run type-check`**; Story **8.1** — Hannibal pre-publish; Story **6.1** ✅; Story **6.6**; Story **5.2**)  
**Owner:** Murdock
...
_No open defects tracked in this table — **BUG-REPORT-001** / **Story 6.7** shipped **2026-04-12** (see [`van/stories.md`](stories.md) bug backlog)._
...
- **Automated evidence:** Full suite **`npm test`** — **176** tests (**all pass**); **`tests/budget-display.test.ts`** — `formatProjectBudgetHours`, `taskEstimatesTotal`, `totalActiveProjectBudget`, **`taskEstimatesTotalFromRollup`**, **`formatTaskStoryNumber`** (**Story 6.3**); **`tests/report-projects-summary-task-estimates.test.ts`** — `getAllProjectsSummary` **SQL roll-up** vs Hannibal **B** + **`startDate`/`endDate`** on summary rows (**Story 6.5**); `report-service` CSV; `report.projectsSummary` smoke (`tests/report-projects-summary-error.test.ts`); **`tests/aggregation-actuals-report-date-range.test.ts`** — explicit wide-range + implicit **All Time** (**BUG-REPORT-001** / **Story 6.7**); **`tests/project-past-end-date.test.ts`** — **Story 6.5** local calendar rule; **`tests/story-6-5-project-past-end-cue-contract.test.ts`** — copy + surface wiring; **`tests/tasks-awaiting-estimates.test.ts`** — **Story 6.2** filter/sort helper; **`tests/task-list-sort.test.ts`** / **`tests/task-sort-storage.test.ts`** — **Story 6.3**; **`tests/task-hide-completed.test.ts`** — **Story 6.4**. **`npm run type-check`** — clean.
...
#### Story 6.2 — Tasks awaiting estimates (project detail) ✅

- **Status (`van/stories.md`):** Story **6.2** acceptance criteria **complete**; shipped **2026-05-05** per Hannibal **Decisions** (`null`-only missing estimates; **`0`** set; **`completed`** excluded; flat hierarchy; **`name`** A→Z; empty card **“All tasks have estimates.”**).
- **Scope to verify:** **`/projects/[id]`** — **Tasks awaiting estimates** card above main task table; rows show name + status + **Add estimate** (opens **`TaskForm`** with estimate field focused); table **Edit** unchanged; after save, list refetches (**`task.listByProject`** invalidation).
- **Automated evidence:** **`tests/tasks-awaiting-estimates.test.ts`** — `isTaskAwaitingEstimate` / `tasksAwaitingEstimatesSorted` (null vs **0**, completed excluded, sort order, mixed rows); **`tests/story-6-2-tasks-awaiting-estimates-contract.test.ts`** — TasksSection card copy/order + modal focus + invalidate wiring. Full suite **`npm test`** — **176** (**all pass**).

#### Story 6.3 — Task list sort & Story # (project detail) ✅

- **Status (`van/stories.md`):** Shipped **2026-05-06** — **`tasks.story_number`**, **`task.listByProject`** `{ sortBy, sortDir }`, **`localStorage`** **`vandura.tasks.sort.{projectId}`**, pipeline **status** order; **partial unique index** — duplicate story # **rejected within** a project; **same** story # **allowed across** projects.
- **Scope to verify:** **`/projects/[id]`** — sortable headers (**Story #**, **Name**, **Status**, **Estimated hours**); **TBD** for unset story # / hours; **6.2** estimates card **name A→Z** independent of table sort; **`/timesheets`** task dropdown uses default sort query input.
- **Automated evidence:** **`tests/task-list-sort.test.ts`** — status asc/desc, **`story_number`** nulls-last + **`id`** tie-break, duplicate blocked / cross-project allowed; **`tests/task-sort-storage.test.ts`**; **`tests/task-validation.test.ts`** — story # **≥ 1**, **`updateTaskDataSchema`** null clears; **`tests/budget-display.test.ts`** — **`formatTaskStoryNumber`**; **`tests/story-6-3-task-list-contract.test.ts`** — TasksSection sort header + **`formatTaskStoryNumber`** wiring. Full suite **`npm test`** — **176** (**all pass**).
- **Hannibal manual (final — before release tag):** Same checklist as **`van/stories.md` → Story 6.3 → Hannibal final review (manual DoD)** — browser sweep on **`/projects/[id]`** (sort, Story #, duplicate modal, persistence, **6.2** card A→Z); optional **6.5** surfaces smoke if not exercised recently.

#### Story 6.4 — Hide completed tasks (project detail main table) ✅

- **Status (`van/stories.md`):** Shipped **2026-05-11** — client filter **`status !== 'completed'`** when toggle on; **`localStorage`** **`vandura.tasks.hideCompleted.{projectId}`**; **6.2** card **not** tied to toggle; explicit empty copy + **Show completed tasks** text action when filter hides all rows; **`/timesheets`** and other routes unchanged.
- **Scope to verify:** **`/projects/[id]`** — **Status** header: sort + eye / eye-off (`aria-pressed`, tooltip); hide **only** **`completed`**; refresh restores preference; **Tasks awaiting estimates** card unchanged.
- **Automated evidence:** **`tests/task-hide-completed.test.ts`** — `visibleTasksForMainTable` ( **`completed`** only, order preserved); **`tests/task-hide-completed-storage.test.ts`** — key prefix + parse; **`tests/story-6-4-hide-completed-contract.test.ts`** — TasksSection vs **6.2** card wiring, a11y, empty state, **`page.tsx`** persistence. Full suite **`npm test`** — **176** (**all pass**).
- **Hannibal manual (final — before release tag):** **`van/stories.md` → Story 6.4 → Hannibal final review (manual DoD)**.
...
#### Story 6.7 — BUG-REPORT-001 / Actuals **All Time** vs planning dates ✅
...
| `tests/aggregation-actuals-report-date-range.test.ts` | **BUG-REPORT-001** / **Story 6.7:** `getActualsVsEstimates` — explicit wide range + implicit **All Time** (`undefined` dates) includes entries after **`projects.endDate`** | ✅ Passing |
| `tests/budget-display.test.ts` | Story 6.1 + **6.3:** `formatProjectBudgetHours`, `taskEstimatesTotal`, `totalActiveProjectBudget`, `taskEstimatesTotalFromRollup`, **`formatTaskStoryNumber`** | ✅ Passing |
...
| `tests/tasks-awaiting-estimates.test.ts` | Story **6.2:** `isTaskAwaitingEstimate`, `tasksAwaitingEstimatesSorted` — **`null`** only vs **`0`**, **`completed`** excluded, **`name`** A→Z | ✅ Passing |
| `tests/story-6-2-tasks-awaiting-estimates-contract.test.ts` | Story **6.2:** `TasksSection` + `TaskForm` — card copy, order vs main table, **Add estimate** focus + `listByProject` invalidation (source contract) | ✅ Passing |
| `tests/task-list-sort.test.ts` | Story **6.3:** SQL **`ORDER BY`** — pipeline **status**, **`story_number`** nulls-last; duplicate story # blocked **per project**, allowed **across** projects | ✅ Passing |
| `tests/task-sort-storage.test.ts` | Story **6.3:** **`parseTaskListSort`** defaults + valid JSON | ✅ Passing |
| `tests/task-hide-completed.test.ts` | Story **6.4:** `visibleTasksForMainTable` — hide **`completed`** only | ✅ Passing |
| `tests/task-hide-completed-storage.test.ts` | Story **6.4:** **`vandura.tasks.hideCompleted.`** key + parse/read contract | ✅ Passing |
| `tests/story-6-4-hide-completed-contract.test.ts` | Story **6.4:** `TasksSection` + `page.tsx` — main table vs **6.2** card, a11y, empty state, persistence wiring | ✅ Passing |
| `tests/story-6-3-task-list-contract.test.ts` | Story **6.3:** `TasksSection` — sortable **Story #** header + **`formatTaskStoryNumber`** display (source contract) | ✅ Passing |
...
**Suite status:** **176** tests — **176** passing, **0** skipped — **36** test files under `tests/*.test.ts` (last full run: **2026-05-11** — includes **Story 6.2**, **6.3**, **6.4**, **6.5**, **6.7** / BUG-REPORT-001).
...
**Automated suite:** **`npm test`** — **176** tests (**all pass**) (**2026-05-11**); includes Story **6.1** rollup tests, Story **6.2** tasks-awaiting-estimates helper, Story **6.3** task-list sort + storage, Story **6.4** hide-completed filter, Story **6.5** past-end helper, Story **6.7** actuals date regression, Story **8.1** bug-report suite, Story **6.6** link test, and prior suites. **`npm run type-check`** and **`npm run lint`** — clean on same pass.
...
**End of Document**  
Last Updated: 2026-05-11 — automated registry **176** tests (**all pass**), **36** `tests/*.test.ts` files; Story **6.1** ✅; Story **6.2** ✅; Story **6.3** ✅; Story **6.4** ✅; Story **6.5** ✅; Story **6.7** / **BUG-REPORT-001** ✅; Story **8.1** Murdock automated ✅ — see **`van/stories.md`**.
